...
> ⚠️ **IMPORTANT**: Replace `YOUR_VPS_IP` with your actual GCP external IP (e.g., `34.93.217.29`). Do NOT use `localhost` or the internal IP (e.g., `10.160.0.2`).
...
### ⚠️ CRITICAL: Load `.env.web` Before Building Web Image

The `NEXT_PUBLIC_*` variables in `.env.web` are **baked into the JavaScript bundle at build time**. You must load them into the shell environment before building:

```bash
cd /opt/quizapp

# Load .env.web variables into shell environment
set -a && source .env.web && set +a

# Now build the web image (vars are baked in)
...
# Pass .env.web directly to docker compose build
docker compose -f docker-compose.prod.yml --env-file .env.web build quiz-web
docker compose -f docker-compose.prod.yml up -d
```
...
# CRITICAL: Reload .env.web into shell environment
set -a && source .env.web && set +a

# Rebuild web with new module specs
docker compose -f docker-compose.prod.yml build quiz-web
...
### CORS Errors / localhost in Browser Requests

If you see errors like:
```
Access to fetch at 'http://localhost:3002/...' from origin 'http://YOUR_IP:3000' blocked by CORS
```

This means the web app was built with `localhost` instead of your VPS IP. Fix:

```bash
cd /opt/quizapp

# 1. Verify .env.web has correct IP (not localhost)
cat .env.web

# 2. Load env vars and rebuild
set -a && source .env.web && set +a
docker compose -f docker-compose.prod.yml build quiz-web
docker compose -f docker-compose.prod.yml up -d quiz-web
