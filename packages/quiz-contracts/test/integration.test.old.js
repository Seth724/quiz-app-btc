...
describe('Integration Tests', function () {
    ...
    it('should complete full teacher-student workflow with separate wallets', async function () {
        ...
        const quizForAttempt = await teacherComputer.sync(quizId);
        ...
        const syncedQuiz = await teacherComputer.sync(quizId);
    ...
    it('should handle multiple students attempting different quizzes', async function () {
        ...
        const attempt1 = await student1Computer.new(attempt_js_1.QuizAttempt, [quiz1Id, student1Computer.getPublicKey()]);
        await attempt1.submitAnswers([1, 2], quiz1.correctAnswers, quiz1.rewardPerCorrect);
        // Student 2 attempts Quiz 2
        const attempt2 = await student2Computer.new(attempt_js_1.QuizAttempt, [quiz2Id, student2Computer.getPublicKey()]);
    ...
    it('should handle concurrent quiz attempts by different students', async function () {
        ...
        const [attempt1, attempt2] = await Promise.all([
            student1Computer.new(attempt_js_1.QuizAttempt, [quiz._id, student1Computer.getPublicKey()]),
            student2Computer.new(attempt_js_1.QuizAttempt, [quiz._id, student2Computer.getPublicKey()])
        ]);
    ...
    it('should handle complete teacher-student lifecycle', async function () {
        ...
        const comprehensiveQuiz = await teacherComputer.new(quiz_js_1.Quiz, [{
        ...
        for (const [index, student] of students.entries()) {
            ...
            const attempt = await studentComputer.new(attempt_js_1.QuizAttempt, [comprehensiveQuiz._id, student.publicKey]);
    ...
    it('should handle edge case: quiz with maximum question count', async function () {
        ...
        const maxQuiz = await teacherComputer.new(quiz_js_1.Quiz, [{
        ...
        const attempt = await student1Computer.new(attempt_js_1.QuizAttempt, [maxQuiz._id, student1Computer.getPublicKey()]);
    ...
    it('should handle quiz deactivation workflow', async function () {
        ...
        const attempt1 = await student1Computer.new(attempt_js_1.QuizAttempt, [quiz._id, student1Computer.getPublicKey()]);
        ...
        try {
            ...
            expect(error.message).to.include('Quiz is not active');
        }
    });
});
