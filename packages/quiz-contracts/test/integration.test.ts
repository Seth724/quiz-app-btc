...
//     // 1. Teacher creates quiz with individual payments for each question
//     console.log('\n📚 Teacher 1 creating quiz with individual payments...')
//     const { quiz, paymentTxIds } = await quizHelper.createQuizWithPayments({
//       title: 'Math Quiz with Individual Payments',
...
//     // Verify payments were created and teacher balance was reduced
//     for (let i = 0; i < paymentTxIds.length; i++) {
//       const initialPayment = await quizHelper.getPaymentDetails(paymentTxIds[i])
//       console.log(`✅ Payment ${i + 1} verified:`, initialPayment._satoshis.toString(), 'satoshis')
//       expect(initialPayment._satoshis).to.equal(1000n) // Each question has 1000n reward
//     }
...
//     // 2. Student completes quiz and receives payment for correctly answered questions
...
//     const { attempt, rewardPaymentTxIds } = await studentHelper.completeQuizWithPayment({
...
//     console.log('💳 Reward payment IDs:', rewardPaymentTxIds)
...
//     // 3. Verify payments were transferred to student
//     if (rewardPaymentTxIds && rewardPaymentTxIds.length > 0) {
//       console.log('\n💸 Verifying payment transfers...')
//       console.log('Number of payments received:', rewardPaymentTxIds.length)

//       for (let i = 0; i < rewardPaymentTxIds.length; i++) {
//         const rewardPayment = await studentHelper.getPaymentDetails(rewardPaymentTxIds[i])
//         console.log(`✅ Payment ${i + 1} amount:`, rewardPayment._satoshis.toString(), 'satoshis')
//         console.log(`✅ Payment ${i + 1} owner:`, rewardPayment._owners[0].slice(0, 10) + '...')
//         console.log('✅ Student public key:', sharedStudent1.publicKey.slice(0, 10) + '...')

//         expect(rewardPayment._satoshis).to.equal(1000n) // Each payment should be 1000n
//         expect(rewardPayment._owners).to.include(sharedStudent1.publicKey)
//       }
//       console.log('✅ Payments successfully transferred to student')
//     } else {
//       throw new Error('No reward payments were created')
//     }
