...
// describe('Main Quiz Flow - Direct Contract Test', function () {
...
//     console.log('\\n🚀 Starting main quiz flow test suite setup...')
...
//     console.log(`✅ Teacher created: ${await teacher.name} (${(await teacher.publicKey).slice(0, 10)}...)`)
...
//     console.log(`✅ Student 1 created: ${await student1.name} (${(await student1.publicKey).slice(0, 10)}...)`)
...
//     console.log(`✅ Student 2 created: ${await student2.name} (${(await student2.publicKey).slice(0, 10)}...)\n`)
...
//     console.log('🎯 Main quiz flow test suite setup complete!\\n')
//   })

//   it('should execute the main quiz flow: teacher creates quiz with payments, students compete for rewards', async function () {
//     this.timeout(120000)
//     console.log('\\n🧪 MAIN TEST: Direct contract implementation of quiz workflow\\n')
...
//       // Create payment with the reward amount (teacher automatically becomes owner)
...
//       expect(await payment._satoshis).to.equal(1000n)
//       expect(await payment._owners).to.include(await teacher.publicKey)
//       const owners = await payment._owners
//       console.log(`✅ Payment ${i + 1} verified: ${await payment._satoshis} sats, owner: ${owners[0].slice(0, 10)}...`)
//     }
//     console.log()
...
//     // Link quiz to teacher separately
...
//     // Create quiz attempt directly
//     const attempt1 = await student1Computer.new(QuizAttempt, [quiz._id, await student1.publicKey]) as QuizAttempt
//     console.log(`📝 Student ${await student1.name} created attempt`)

//     // Submit answers and get score
//     const quizCorrectAnswers = await quiz.correctAnswers
//     const quizRewardPerCorrect = await quiz.rewardPerCorrect
//     await attempt1.submitAnswers([1, 0], quizCorrectAnswers, quizRewardPerCorrect)
//     console.log(`✅ Answers submitted, score: ${await attempt1.score}, reward: ${await attempt1.rewardEarned}`)

//     // Add student to attempted list
//     await quiz.addAttemptedStudent(await student1.publicKey)
...
//     // Process rewards for correctly answered questions one by one
//     const rewardPaymentIds1: string[] = []
//     for (let i = 0; i < [1, 0].length; i++) {
//       const isCorrect = [1, 0][i] === quizCorrectAnswers[i]

//       if (isCorrect) {
//         // Try to claim the reward for this question
//         const wasClaimed = await quiz.claimQuestionReward(i, await student1.publicKey)

//         if (wasClaimed) {
//           // The student was the first to answer this question correctly, so they get the reward
//           const paymentTxId = await quiz.getPaymentTxIdForQuestion(i)

//           if (paymentTxId) {
//             try {
//               // Mine block before syncing to avoid script stack size issues
//               if (network === 'regtest') {
//                 await ContractUtils.mineBlockFromRPCClient(student1Computer)
//                 await new Promise(resolve => setTimeout(resolve, 500))
//               }

//               // Sync the payment from student's computer to get access
//               const payment = await student1Computer.sync(paymentTxId) as Payment

//               // Transfer ownership to the student
//               const student1Pub = await student1.publicKey
//               await payment.transfer(student1Pub)

//               console.log(`✅ Payment for question ${i + 1} transferred to student ${student1Pub.slice(0, 10)}...`)

//               rewardPaymentIds1.push(payment._id)

//               // Mine block after payment transfer
//               if (network === 'regtest') {
//                 await ContractUtils.mineBlockFromRPCClient(student1Computer)
//                 await new Promise(resolve => setTimeout(resolve, 1000))
//               }
...
//     console.log(`   Score: ${await attempt1.score}/2`)
//     console.log(`   Reward earned: ${await attempt1.rewardEarned} satoshis`)
//     console.log(`   Payments received: ${rewardPaymentIds1.length}\n`)

//     // Verify student 1's attempt
//     expect(await attempt1.score).to.equal(1)
//     expect(await attempt1.rewardEarned).to.equal(1000n)
//     expect(rewardPaymentIds1.length).to.equal(1)

//     // Verify student 1 got payment for Q1
//     if (rewardPaymentIds1.length > 0) {
//       // Add delay before syncing payment
//       if (network === 'regtest') {
//         await new Promise(resolve => setTimeout(resolve, 1000))
//       }

//       const payment = await student1Computer.sync(rewardPaymentIds1[0]) as Payment
...
//     const rewardPaymentIds2: string[] = []
//     for (let i = 0; i < [1, 2].length; i++) {
//       const isCorrect = [1, 2][i] === quizCorrectAnswers[i]
//       if (isCorrect) {
//         const wasClaimed = await quiz.claimQuestionReward(i, await student2.publicKey)
//         if (wasClaimed) {
//           const paymentTxId = await quiz.getPaymentTxIdForQuestion(i)
//           if (paymentTxId) {
//             try {
...
//               // Sync the payment from student's computer to get access
//               const payment = await student2Computer.sync(paymentTxId) as Payment

//               // Transfer ownership to the student
//               const student2Pub = await student2.publicKey
//               await payment.transfer(student2Pub)

//               console.log(`✅ Payment for question ${i + 1} transferred to student ${student2Pub.slice(0, 10)}...`)

//               rewardPaymentIds2.push(payment._id)
...
//     console.log(`   Payments received: ${rewardPaymentIds2.length}\\n`)
...
//     expect(rewardPaymentIds2.length).to.equal(1) // Only Q2 payment (Q1 already claimed)

//     // Verify student 2 got payment for Q2 only
//     if (rewardPaymentIds2.length > 0) {
//       // Add delay before syncing payment
//       if (network === 'regtest') {
//         await new Promise(resolve => setTimeout(resolve, 1000))
//       }

//       const payment = await student2Computer.sync(rewardPaymentIds2[0]) as Payment
...
//     // Create a fresh payment with the teacher as owner (automatic)
//     const payment = await teacherComputer.new(Payment, [500n])
//     console.log(`Created payment: ${await payment._id}\n`)
...
//       teacherPublicKey: await teacher.publicKey,
//       paymentTxIds: [await payment._id]
//     }]) as Quiz
...
//     const initialPayment = await teacherComputer.sync(await payment._id) as Payment
//     const initialOwners = await initialPayment._owners
//     expect(initialOwners).to.include(await teacher.publicKey)
//     console.log(`✅ Initial: Payment owned by teacher`)
//     console.log(`   Owner: ${initialOwners[0].slice(0, 10)}...\n`)

//     // Student 1 attempts and gets correct answer
//     const attempt = await student1Computer.new(QuizAttempt, [await quiz._id, await student1.publicKey]) as QuizAttempt
//     console.log(`Student 1 attempted quiz`)

//     const ownerQuizCorrect = await quiz.correctAnswers
//     const ownerQuizReward = await quiz.rewardPerCorrect
//     await attempt.submitAnswers([1], ownerQuizCorrect, ownerQuizReward)
//     console.log(`Score: ${await attempt.score}`)
...
//       const isCorrect = [1][i] === ownerQuizCorrect[i]
//       if (isCorrect) {
//         const wasClaimed = await quiz.claimQuestionReward(i, await student1.publicKey)
//         if (wasClaimed) {
//           const paymentTxId = await quiz.getPaymentTxIdForQuestion(i)
...
//               // Sync the payment from student's computer to get access
//               const studentPayment = await student1Computer.sync(paymentTxId) as Payment

//               // Transfer ownership to the student
//               const s1Pub = await student1.publicKey
//               await studentPayment.transfer(s1Pub)

//               rewardPaymentIds.push(await studentPayment._id)
...
//     console.log(`Payments received: ${rewardPaymentIds.length}\\n`)
...
//     expect(rewardPaymentIds.length).to.equal(1)
//     if (rewardPaymentIds.length > 0) {
//       const transferredPayment = await student1Computer.sync(rewardPaymentIds[0]) as Payment
//       const transferredOwners = await transferredPayment._owners
//       expect(transferredOwners).to.include(await student1.publicKey)
//       expect(transferredOwners).to.not.include(await teacher.publicKey)
//       console.log(`✅ After attempt: Payment owned by student 1`)
//       console.log(`   Owner: ${transferredOwners[0].slice(0, 10)}...`)
...
//     // Create a single payment (teacher auto-owns it)
...
//     const wrongQuizCorrect = await quiz.correctAnswers
//     const wrongQuizReward = await quiz.rewardPerCorrect
//     await attempt.submitAnswers([0], wrongQuizCorrect, wrongQuizReward) // Wrong answer
//     console.log(`Score: ${await attempt.score}`)
//     console.log(`Reward earned: ${await attempt.rewardEarned}`)
...
//       const isCorrect = [0][i] === wrongQuizCorrect[i]
//       if (isCorrect) {
//         const wasClaimed = await quiz.claimQuestionReward(i, await student1.publicKey)
//         if (wasClaimed) {
//           const paymentTxId = await quiz.getPaymentTxIdForQuestion(i)
//           if (paymentTxId) {
//             // Transfer the existing payment to student
//             const payment = await student1Computer.sync(paymentTxId) as Payment
//             await payment.transfer(await student1.publicKey)

//             rewardPaymentIds.push(await payment._id)
...
//     // Verify zero rewards
//     expect(await attempt.score).to.equal(0)
//     expect(await attempt.rewardEarned).to.equal(0n)
//     expect(rewardPaymentIds.length).to.equal(0)
